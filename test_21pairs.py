...
from net import NestFuse_light2_nodense, Fusion_network_Transformer
import utils
...
def load_model(path_auto, path_fusion, flag_img):
	if flag_img is True:
		nc = 3
	else:
		nc =1
	...
	# nest_model = NestFuse_light2_residual(nb_filter, input_nc, output_nc, deepsupervision=False)
	nest_model.load_state_dict(torch.load(path_auto))

	fusion_model = Fusion_network_Transformer(nb_filter)
...
def run_demo(nest_model, fusion_model, infrared_path, visible_path, output_path_root, name_ir, flag_img):
...
def main():
	...
	output_path_root = "./outputs/21pairs/"
	# output_path_root = "./outputs/features/transformer_sal20_f_feture/"
	...
	with torch.no_grad():
		temp = 'mfst'
		output_path_list = 'fused_' + temp + '_21'
		output_path1 = output_path_root + output_path_list + '/'
		if os.path.exists(output_path1) is False:
			os.mkdir(output_path1)
		output_path = output_path1
		# load network
		path_fusion = path_fusion_root + 'Transformer_sal21_Final_epoch_3.model'
		model, fusion_model = load_model(path_auto, path_fusion, flag_img)
		...
		for i in range(num):
			...
			run_demo(model, fusion_model, infrared_path, visible_path, output_path, name_ir, flag_img)
		print('Done......')
